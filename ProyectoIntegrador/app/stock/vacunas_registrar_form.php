...
if(!isset($_GET['id'])) {
    ...
    if (isset($_POST['registrar_form'])) {
        ...
        if (!$error) {
            if ($registrar_stockvacuna->execute()) {
                $successmsg = "¡Registro exitoso!";

            } else {
                $errormsg = "Error al registrar dosis.";
                print_r ($registrar_stockvacuna->errorInfo()); // Developer Mode
            }
        }
    }
?>
    ...
    <form id="editform" class="col-sm-12" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="editform">
        <div class="col-sm-6 form-group">
            ...
            <?php
                ...
                $dropdown = "<select name='id_vacuna' class='form-control'>";
